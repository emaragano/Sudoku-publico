...
def number_in_row(grid, row, number):
    """
    Verificar si un número se encuentra en la fila.
    """
    return number in grid[row]
def number_in_col(grid, col, number):
    """
    Verificar si un número se encuentra en la columna.
    """
    return number in (row[col] for row in grid)
def number_in_box(grid, row, col, number):
    """
    Verificar si un número se encuentra en la matriz a la que
    corresponde la posición.
    """
    # Obtener la matriz a la que pertenece el número.
    box_row, box_col = box_by_pos(row, col)
    # Construir una lista con los números en la matriz.
...
def get_possible_numbers(grid, row, col):
    """
    Entregar números posibles para una determinada posición.
    """
...
def main():
    while True:
        # Los ceros son casilleros vacíos.
        grid = [
            [5, 3, 0, 0, 7, 0, 0, 0, 0],
            [6, 0, 0, 1, 9, 5, 0, 0, 0],
            [0, 9, 8, 0, 0, 0, 0, 6, 0],
        ...
        while True:
            ...
            # Obtener una cantidad de numeros posibles para llenar 
            ...
            # Remover los ceros y ordenar por la cantidad de 
            # probabilidades.
            ...
            if possible_numbers:
                ...
                # Obtener un número aleatorio de la 
        ...
        # Chequear si dió resultado: si no hay más 
        # ceros en la tabla entonces el Sudoku está resuelto.
